  ...
  module Hosts
    class Base
      ...
      def additions_file
        ...
        raise VagrantParallelsTools::IsoPathAutodetectionError if !path || path.empty?
        ...
        if file_match? path
          ...
          raise VagrantParallelsTools::DownloadingDisabledError.new(:from => path) if options[:no_remote]
      ...
      protected
        ...
        def download(path)
          @download = VagrantParallelsTools::Download.new(path, @env.tmp_path, :ui => @env.ui)
          @download.download!

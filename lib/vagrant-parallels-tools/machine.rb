module VagrantParallelsTools
  class Machine
    ...
    def initialize vm, options
      ...
      @logger = Log4r::Logger.new("vagrant::plugins::prltools-machine")
      @logger.debug("initialize prltools machine for VM '#{vm.name}' (#{vm.to_s})")
    ...
    def run
      ...
      while (command = runlist.shift)
        ...
        if !self.send(command)
          env.ui.error('vagrant_parallels_tools.machine_loop_guard', :command => command, :state => current_state)
          return false
        end
    ...
    def install
      return env.ui.warn(I18n.t("vagrant_parallels_tools.skipped_installation")) if options[:no_install] && !options[:force]
      guest_additions_state.trigger :install
    end

    def rebuild
      return env.ui.warn(I18n.t("vagrant_parallels_tools.skipped_rebuild")) if options[:no_install] && !options[:force]

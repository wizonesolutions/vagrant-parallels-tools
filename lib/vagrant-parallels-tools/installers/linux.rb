  ...
  module Installers
    ...
    class Linux < Base
      ...
      def install(opts=nil, &block)
        env.ui.warn I18n.t("vagrant_parallels_tools.errors.installer.generic_linux_installer") if self.class == Linux
        upload(iso_file)
      ...
      # This overrides {VagrantParallelsTools::Installers::Base#guest_version}
      # to also query the `VBoxService` on the host system (if available)
      # for it's version.
      # In some scenarios the results of the Parallels Desktop driver and the
      ...
      def guest_version(reload = false)
        ...
        communicate.sudo('VBoxService --version', :error_check => false) do |type, data|
          if (v = data.to_s.match(/^(\d+\.\d+.\d+)/)) && driver_version != v[1]
            @env.ui.warn(I18n.t("vagrant_parallels_tools.guest_version_reports_differ", :driver => driver_version, :service => v[1]))
            @guest_version = v[1]
          end
        end
        @guest_version
...
VagrantParallelsTools::Installer.register(VagrantParallelsTools::Installers::Linux, 2)
